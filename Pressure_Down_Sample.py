...
press = np.empty([0,3])
curr_low=pd.read_hdf(store,"Combined",where="index=0")["Pressure"][0]
curr_high=0.0
if nrows > 500000:
    prev = 0
    for i in tqdm(range(0,nrows,500000)):
        df_press = pd.read_hdf(store,"Combined",where="index<="+str(i)+" and index>"+str(prev))
        if len(df_press) > 0:
            ind = prev + 1
    ...
    df_press = pd.read_hdf(store,"Combined")
    ind = 0
    prev_sec = 0
    for j in tqdm(df_press.Time):
        if j.second > prev_sec:
...
df_AGM_List.columns = ["Name","Chainage","Elevation","Day","Time","Type"]
